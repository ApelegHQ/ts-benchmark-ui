...
export const unmarshalSuiteState_ = (data: unknown): ISuiteState => {
	...
	if (data.length !== 5) throw new Error('SuiteState length mismatch');
	const [name, numericTriple, setupCodeArr, functionsArr, extraCodeArr] =
		data;
	...
	assertArray(extraCodeArr, 'SuiteState.extraCode');
	if (extraCodeArr.length !== 1) {
		throw new Error('SuiteState.extraCode expected single-item array');
	}
	assertString(extraCodeArr[0], 'SuiteState.extraCode[0]');
